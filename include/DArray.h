...
// Insert several items to a dynamic array
#define DArray_insert_many(da, new_items, new_items_count, index)                                                                    \
    do                                                                                                                               \
    {                                                                                                                                \
        if ((da)->count + (new_items_count) > (da)->capacity)                                                                        \
        {                                                                                                                            \
        ...
        memmove((da)->items + ((index) + (new_items_count)), (da)->items + (index), ((da)->count - (index)) * sizeof(*(da)->items)); \
...
#define DArray_remove(da, index)                                                                                     \
    ...
    {                                                                                                                \
        if ((index) >= (da)->count)                                                                                  \
        {                                                                                                            \
            DEBUG_ERROR("remove of index %d from darray of count %d", (int)(index), (int)(da)->count);               \
            break;                                                                                                   \
        }                                                                                                            \
...
#define DArray_remove_from(da, index)                                                                    \
    ...
    {                                                                                                    \
        if ((index) >= (da)->count)                                                                      \
        {                                                                                                \
            DEBUG_ERROR("remove from index %d from darray of count %d", (int)(index), (int)(da)->count); \
            break;                                                                                       \
        }                                                                                                \
...
#define DArray_remove_many(da, index, how_many)                                                                                            \
    ...
    {                                                                                                                                      \
        ...
        {                                                                                                                                  \
            DEBUG_ERROR("remove of index %d count %d from darray of count %d", (int)(index), (int)(how_many), (int)(da)->count);           \
            break;                                                                                                                         \
        }                                                                                                                                  \
